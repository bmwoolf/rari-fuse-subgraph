...
export function handleRepayBorrow(event: RepayBorrow): void {
  // log.warning(`🚨 cToken line 128, begin handleRepayBorrow()`, []);
  const entity = CtokenSchema.load(event.address.toHexString());
  //entity.totalBorrows = event.params.totalBorrows;
  ...
  updateCtoken(event, entity, event.address);
  entity.save();
  // log.warning(`🚨 cToken line 168, end handleRepayBorrow()`, []);
}

export function handleMint(event: Mint): void {
  // log.warning(`🚨 cToken line 176, begin handleMint()`, []);
  const entity = CtokenSchema.load(event.address.toHexString());

  updateCtoken(event, entity, event.address);
  entity.save();
  // log.warning(`🚨 cToken line 184, end handleMint()`, []);
}

export function handleLiquidateBorrow(event: LiquidateBorrow): void {
  // log.warning(`🚨 cToken line 191, begin handleLiquidateBorrow()`, []);
  const entity = CtokenSchema.load(event.address.toHexString());
  ...
  pool.save();
  entity.save();
  // log.warning(`🚨 cToken line 210, end handleLiquidateBorrow()`, []);
}
...
function updateCtoken(
  ...
  if (!_supplyRatePerBlock.reverted) {
    ...
    log.warning("🚨 updateCtoken line 494 updating supplyAPY to {} from {}", [
      convertMantissaToAPY(
  ...
  log.warning(`🚨 cToken line 515, end updateCtoken()`, []);
}
